...
namespace img_lib 
{
    ...
    void ConvertScanlineToJPEG(std::vector<JSAMPLE>& buff, int y, const Image& image) 
    {
        for (int x = 0; x < image.GetWidth(); ++x) 
        {    
            const Color component = image.GetPixel(x, y);
            // Конвертировать из byte в JSAMPLE можно через static_cast
            buff[x * 3 + 0] = static_cast<JSAMPLE>(component.r);
            buff[x * 3 + 1] = static_cast<JSAMPLE>(component.g);
            buff[x * 3 + 2] = static_cast<JSAMPLE>(component.b);
        }
    }

    // В эту функцию вставлен код примера из библиотеки libjpeg.
    // Измените его, чтобы адаптировать к переменным file и image.
    // Уберите вызов функции jpeg_set_quality(), будет использовано качество по умолчанию
    bool SaveJPEG(const Path& file, const Image& image) 
    {
        ...
        */
        ...
        // Для хранения конвертированных значений можно использовать временный буфер std::vector<JSAMPLE>
        ...
        while (cinfo.next_scanline < cinfo.image_height) 
        {
            ...
            */
            ...
            ConvertScanlineToJPEG(buffer, y, image);
    ...
    void SaveScanlineToImage(const JSAMPLE* row, int y, Image& out_image) 
    {
        ...
        {   
            /*
                Первый пиксель строки используется для заполнения первых трёх элементов временного буфера — от нулевого до второго, 
                второй — для элементов от третьего до пятого
            */
