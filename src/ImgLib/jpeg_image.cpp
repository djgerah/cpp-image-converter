...
void SaveScanlineToImage(const JSAMPLE *row, int y, Image &out_image)
{
    ...
    {
        /*
            Первый пиксель строки используется для заполнения первых трёх элементов временного буфера — от нулевого до
           второго, второй — для элементов от третьего до пятого
        */
