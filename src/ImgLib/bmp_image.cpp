...
PACKED_STRUCT_BEGIN BitmapFileHeader
{
    ...
    char bfType[2] = {'B', 'M'};
    // Суммарный размер заголовка и данных — 4 байта, беззнаковое целое. Размер данных определяется как отступ,
    // умноженный на высоту изображения
    uint32_t bfSize = 0;
...
PACKED_STRUCT_BEGIN BitmapInfoHeader
{
    ...
    // Горизонтальное разрешение, пикселей на метр — 4 байта, знаковое целое. Нужно записать 11811, что примерно
    // соответствует 300 DPI
    int32_t biXPelsPerMeter = 11811;
    // Вертикальное разрешение, пикселей на метр — 4 байта, знаковое целое. Нужно записать 11811, что примерно
    // соответствует 300 DPI
